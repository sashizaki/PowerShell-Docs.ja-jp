...
このサンプルでは、パラメーターを持つエイリアスがあり、パラメーターがワイルドカード文字をサポートするコマンドレットを記述する方法を示します。 このコマンドレットと似ています、`Stop-Process`コマンドレットの Windows PowerShell 2.0 で提供されます。

### <a name="how-to-build-the-sample-by-using-visual-studio"></a>Visual Studio を使用してサンプルをビルドする方法。

1. インストールされている Windows PowerShell 2.0 sdk では、StopProcessSample03 フォルダーに移動します。 既定の場所は、C:\Program Files (x86) \Microsoft SDKs\Windows\v7.0\Samples\sysmgmt\WindowsPowerShell\csharp\StopProcessSample03 します。
...
- パラメーターの宣言にエイリアスを追加する.

- パラメーターにワイルドカードのサポートを追加します。

## <a name="example"></a>例

このサンプルでは、パラメーターのエイリアスを宣言し、ワイルドカードをサポートする方法を示します。
