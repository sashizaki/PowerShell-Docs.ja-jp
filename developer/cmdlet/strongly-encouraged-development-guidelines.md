...
ユーザー エクスペリエンスを強化するために、コマンドレット名の選択した名詞は単数形にあります。 たとえば、名前を使用して`Get-Process`の代わりに**Get-Processes**します。 コマンドレットは、1 つ以上の項目に対して操作を実行する可能性が高い場合でも、すべてのコマンドレット名にはこの規則に従うことをお勧めします。
