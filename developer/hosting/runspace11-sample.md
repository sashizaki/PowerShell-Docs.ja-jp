...
このサンプルは、使用する方法を示します、 [System.Management.Automation.Proxycommand](/dotnet/api/System.Management.Automation.ProxyCommand)クラスを既存のコマンドレットを呼び出しますが、使用可能なパラメーターのセットを制限するプロキシ コマンドを作成します。 このプロキシ コマンドは、制約付き実行空間の作成に使用される最初のセッション状態に追加されます。 つまり、ユーザーはプロキシ コマンドを使用しないとコマンドレットの機能にアクセスできません。
...
- 作成、 [System.Management.Automation.Commandmetadata](/dotnet/api/System.Management.Automation.CommandMetadata)既存のコマンドレットのメタデータを記述するオブジェクト。
...
- コマンドレットのパラメーターを削除するコマンドレットのメタデータを変更します。

- コマンドレットを追加する、 [System.Management.Automation.Runspaces.Initialsessionstate](/dotnet/api/System.Management.Automation.Runspaces.InitialSessionState)オブジェクトと、コマンドレットをプライベートにします。

- プロキシ関数を作成すると、既存のコマンドレットを呼び出しますが、パラメーターの制限のセットのみを公開します。

- 最初のセッション状態をプロキシ関数を追加します。
...
- プライベート コマンドレットと、プロキシを使用して関数を呼び出し、 [System.Management.Automation.Powershell](/dotnet/api/system.management.automation.powershell)を制約付き実行空間を示すオブジェクト。

## <a name="example"></a>例

これには、プロキシ コマンドを制約付き実行空間を示すためにプライベート コマンドレットが作成されます。
