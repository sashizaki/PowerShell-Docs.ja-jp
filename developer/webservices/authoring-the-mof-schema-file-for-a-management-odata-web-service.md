...
次のファイルは、サービスとプロセスのリソースを定義します。 これらの各リソースは、Windows PowerShell コマンドレットのセットによって管理できるオブジェクトに対応します。 プロパティは、これらのコマンドレットによって使用されるパラメーターに対応します。

2 つのリソースには、複合型のプロパティが含まれています。 クラスを使用して変更として、複合型は定義されて、`ComplexType`修飾子。
