...
すべての管理の OData web サービスを作成するための手順を完了したら後、は、IIS で web アプリケーションとしてデプロイする必要があります。

## <a name="deploying-the-web-service"></a>Web サービスのデプロイ

Management OData web サービスをデプロイするには、次の手順を完了します。

1. ディレクトリを作成、IIS 下でアプリケーションを web`WWWRoot`ディレクトリ。

2. MOF スキーマ ファイル、XML スキーマ ファイルをエクスポートする Dll をコピー、 [Microsoft.Management.Odata.Customauthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)と[System.Management.Automation.Remoting.Pssessionconfiguration](/dotnet/api/System.Management.Automation.Remoting.PSSessionConfiguration)インターフェイス、およびアプリケーションのディレクトリに web.config ファイル。
...
次の Windows PowerShell スクリプトでは、Management OData web サービスをデプロイする方法を示します。
