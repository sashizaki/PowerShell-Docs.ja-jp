...
---
# <a name="resource-mapping-schema"></a>リソース マッピング スキーマ

Management OData IIS 拡張機能では、XML ファイルを使用して、リソースのマッピングを定義します。 次の XSD では、これらのファイルで使用されるスキーマを定義します。

## <a name="resource-mapping-file-xsd"></a>マッピングの XSD リソース ファイル

次の XSD では、Management OData IIS 拡張機能リソースのマッピング ファイルのスキーマを定義します。
