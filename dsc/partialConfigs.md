...
プッシュ モードの部分構成用の LCM を構成するには、各部分構成に 1 つの **PartialConfiguration** ブロックを使用して **DSCLocalConfigurationManager** 構成を作成します。 LCM の構成の詳細については、「[ローカル構成マネージャーの構成](https://technet.microsoft.com/library/mt421188.aspx)」をご覧ください。
...
プル サーバーから複数の部分構成をプルする場合、構成ドキュメントの名前は `<ConfigurationName>.mof` (_ConfigurationName_ は部分構成の名前)、または `<ConfigurationName>.<NodeName>.mof` (_ConfigurationName_ は部分構成の名前、_NodeName_ はターゲット ノードの名前) のどちらかにすることができます。
