...
Desired State Configuration (DSC) は、Common Information Model (CIM) を記述するために管理オブジェクト フォーマット (MOF) という業界構文を使用して、構成データの展開と管理を可能にする管理プラットフォームです。 オープン ソース プロジェクトの Open Management Infrastructure (OMI) は、Linux とネットワーク ハードウェア オペレーティング システムを含むプラットフォーム全体でこれらの標準をさらに開発するために存在します。 詳細については、[MOF 仕様にリンクされている DMTF ページ](http://dmtf.org/standards/cim)、および [OMI のドキュメントとソース](https://collaboration.opengroup.org/omi/documents.php)を参照してください。
...
DNS レコードの名前を選択する場合は、ソリューションのアーキテクチャに注意してください。 負荷分散を使用する場合、HTTPS 経由でトラフィックを保護するために使用される証明書で DNS レコードと同じ名前を共有する必要があります。 同様に、高可用性ファイル共有を使用する場合、クラスターの仮想名が使用されます。
...
高可用性展開 |クライアントが負荷分散ソリューションを通じて接続される場合は、仮想 IP のホスト名と、そのホスト名を参照する CNAME レコードを作成します。 DNS ラウンド ロビンがプル サーバーでのクライアント要求の分散に使用される場合は、展開されたすべてのプル サーバー インスタンスのホスト名が含まれるように名前レコードを構成する必要があります。|

詳細については、[Windows Server での DNS ラウンド ロビンの構成](https://technet.microsoft.com/en-us/library/cc787484(v=ws.10).aspx)に関するページを参照してください。
...
プル サーバー役割は、高可用性アーキテクチャに展開できます。 Web サービスの役割を負荷分散することができ、DSC モジュールと DSC 構成を含むファイルおよびフォルダーを高可用性記憶装置に配置することができます。

構成とモジュールがターゲット ノードに配信されると、テストを実行し、構成を設定するために必要なすべてのデータが各ノードにローカルに格納されることに注意してください。 プル サーバーからの変更のみが配信されます。 プル サーバーのサービス停止は、展開がアクティブでない限り、妨げにはなりません。  通常、高可用性は最大規模の環境でのみ保証されます。

高可用性プル サーバー環境を構成する場合、複数のサーバー ノードでのクライアント要求の分散方法や、これらのノードでの必要なサーバー ファイルの共有方法に関する意思決定が必要になります。
...
複数のサーバーがプル サーバーとして構成され、それらのサーバーで接続の負荷分散が行われる高可用性シナリオでは、サーバーで使用可能なリソースと構成が同じであることが重要です。 これを実現する最善の方法は、クラスター化されたファイル共有などの高可用性の場所でこのコンテンツを格納することです。 各サーバーの構成で共有の場所を指定できます。共有記憶域オプションの詳細については、「アプリケーション データに対応するスケールアウト ファイル サーバーの概要」を参照してください。

計画タスク|
---|
高可用性共有をホストする際にどのようなソリューションが使用されますか?|
新しい高可用性共有の要求は誰が処理しますか?|
高可用性共有が使用可能になるまでの平均所要時間はどれくらいですか?|
記憶域および/またはクラスタリング担当チームではどのような情報が必要ですか?|
...
構成 GUID の計画についても、プル サーバー展開を検討する際に考慮する必要があります。 GUID の処理方法に関する特定の要件はありませんが、プロセスは環境ごとに一意である可能性があります。 プロセスは、単純なものから複雑なものまでさまざまです。たとえば、セントラルに格納されている CSV ファイル、単純な SQL テーブル、CMDB、別のツールまたはソフトウェア ソリューションとの統合が必要な複雑なソリューションなどです。 2 つの一般的な方法を以下に示します。
 ...
 - **サーバーの役割ごとに GUID を割り当てる** — Web サーバーなど、同じ機能を実行するすべてのサーバーで同じ GUID を使用して、必要な構成データを参照します。  多くのサーバーで同じ GUID を共有している場合、それらすべてのサーバーは、構成の変更と同時に更新されることに注意してください。
...
注: 現時点では、`xPSDesiredStateConfiguation` DSC モジュールでは、サーバーのロケールとして EN-US を設定する必要があります。
