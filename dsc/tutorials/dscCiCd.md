...
Windows ビルド エージェントをインストールし実行する方法については、「[Deploy an agent on Windows (Windows 上にエージェントを展開する)](/azure/devops/pipelines/agents/v2-windows)」をご覧ください。
...
1. TFS で、[新しいチーム プロジェクトを作成](/azure/devops/organizations/projects/create-project)し、Demo_CI という名前にします。
...
1. クライアント コンピューターで、次のコマンドを使用して、いま TFS で作成したリポジトリにリモートを追加します。
   ...
   この URL の場所がわからない場合は、「[Clone an existing Git repo (既存の Git リポジトリを複製する)](/azure/devops/repos/git/clone)」をご覧ください。
...
最初のリソース ブロックでは、構成で **WindowsFeature** を呼び出し、DNS 機能が有効になっていることを確認します。
...
[Start-DscConfiguration](/powershell/module/psdesiredstateconfiguration/start-dscconfiguration) コマンドレットを呼び出して、`TestAgent1` で構成を実行します。
...
[リリース定義の操作](https://www.visualstudio.com/en-us/docs/build/actions/work-with-release-definitions)) し、次のように構成します。
