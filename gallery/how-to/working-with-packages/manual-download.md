...
Powershell ギャラリーでは、PowerShellGet コマンドレットを使用せずに、Web サイトから直接パッケージのダウンロードをサポートしています。 パッケージは NuGet パッケージ (.nupkg) ファイルとしてダウンロードされます。これは内部リポジトリに簡単にコピーすることができます。
