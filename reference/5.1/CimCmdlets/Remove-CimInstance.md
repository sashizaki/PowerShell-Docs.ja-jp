...
指定された値に二重引用符 (")、単一引用符 (')、または円記号 () が含まれている場合は \\ 、円記号 () を付けることで、これらの文字をエスケープする必要があり \\ ます。 指定された値が WQL LIKE 演算子を使用する場合は、次の文字を角かっこ () で囲むことによってエスケープする必要があります ( \[ \] ): パーセント (%)、アンダースコア (_)、または左角かっこ ( \[ )。
