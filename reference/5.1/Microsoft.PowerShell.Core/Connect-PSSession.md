...
接続が切断されたユーザーが管理する Windows PowerShell **セッション (** **PSSession** ) に再接続します。
...
既定では、Windows PowerShell は接続をリダイレクトしませんが、ユーザーはこのパラメーターを使用して接続をリダイレクトできます。
...
Windows PowerShell リモート処理用の既定のポートを使用するには、HTTP の場合は 5985、HTTPS の場合は 5986 を指定します。
...
このリスナーを構成するには、Windows PowerShell プロンプトで次の 2 つのコマンドを入力します。
