...
### 例 1: スレッドのアパートメント状態を変更する

```
PS C:\> Set-PSSessionConfiguration -Name "MaintenanceShell" -ThreadApartmentState STA
```

このコマンドは、MaintenanceShell 構成のスレッド アパートメント状態を STA に変更します。
変更は、 **WinRM** サービスを再起動すると有効になります。

### 例 2: セッション構成を作成して変更する
