...
`New-ModuleManifest` 次のようにエンコードされたモジュールマニフェスト ( `.psd1` ) ファイルを作成します。

- Windows で実行されている PowerShell 6.0 は、 **UTF16** を使用します。
- Windows 以外のプラットフォームで実行されている PowerShell 6.0 では、 **UTF8NoBOM** を使用します。
- Windows および Windows 以外のプラットフォームで実行されている PowerShell 6.1 以降では、 **UTF8NoBOM** が使用されます。
