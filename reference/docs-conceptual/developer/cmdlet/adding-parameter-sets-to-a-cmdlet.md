...
2つのパラメーターセットを使用して異なる機能を定義するコマンドレットの例として、Windows PowerShell によって提供される @no__t 0 のコマンドレットがあります。 このコマンドレットは、ユーザーが `List` または `LogName` パラメーターを指定すると、異なる情報を返します。 @No__t-0 パラメーターが指定されている場合、コマンドレットは、特定のイベントログ内のイベントに関する情報を返します。 @No__t-0 パラメーターが指定されている場合、コマンドレットはログファイル自体に関する情報 (含まれているイベント情報ではありません) を返します。 この場合、`List` および `LogName` パラメーターは、2つの異なるパラメーターセットを識別します。
...
最後の点を示すために、この Stop Proc コマンドレットは、3つのパラメーターセット (`ProcessName`、`ProcessId`、`InputObject`) を使用します。 これらの各パラメーターセットには、他のパラメーターセットに含まれていないパラメーターが1つあります。 パラメーターセットは他のパラメーターを共有できますが、コマンドレットでは、一意のパラメーター `ProcessName`、`ProcessId`、`InputObject` を使用して、Windows PowerShell ランタイムが使用するパラメーターのセットを識別します。
...
このコマンドレットは、コマンドレットへの入力として必要な3つのパラメーター (これらのパラメーターはパラメーターセットも定義します @no__t) と、コマンドレットが出力オブジェクトを送信するかどうかを決定する `PassThru` パラメーターを定義します。パイプラインを使用します。 既定では、このコマンドレットはパイプラインを介してオブジェクトを渡しません。 これらの最後の2つのパラメーターの詳細については、「[システムを変更するコマンドレットを作成](./creating-a-cmdlet-that-modifies-the-system.md)する」を参照してください。

### <a name="declaring-the-name-parameter"></a>Name パラメーターの宣言

この入力パラメーターを使用すると、停止するプロセスの名前をユーザーが指定できます。 @No__t- [0 属性キーワード](/dotnet/api/System.Management.Automation.ParameterAttribute)は、このパラメーターに設定されている @no__t 2 つのパラメーターを指定していることに注意してください。
...
この入力パラメーターを使用すると、停止するプロセスの識別子をユーザーが指定できます。 @No__t-0 属性キーワードは、@no__t 2 つのパラメーターセットを指定することに[注意して](/dotnet/api/System.Management.Automation.ParameterAttribute)ください。
...
この入力パラメーターを使用すると、停止するプロセスに関する情報を含む入力オブジェクトをユーザーが指定できます。 @No__t- [0 属性キーワード](/dotnet/api/System.Management.Automation.ParameterAttribute)は、このパラメーターに設定されている @no__t 2 つのパラメーターを指定していることに注意してください。
...
コマンドレットが Windows PowerShell に登録されている場合は、コマンドラインで実行してテストします。 ここでは、`ProcessId` パラメーターと @no__t パラメーターを使用してパラメーターセットをテストしてプロセスを停止する方法を示すテストをいくつか示します。
