...
Stop Proc コマンドレットは、3つのパラメーター (`Name`、`Force`、`PassThru`) を定義します。 これらのパラメーターの定義の詳細については、「[システムを変更するコマンドレットを作成](./creating-a-cmdlet-that-modifies-the-system.md)する」を参照してください。
