...
コマンドレットを使用して、動詞の完全な一覧を表示でき `Get-Verb` ます。
...
`New`動詞は、新しいリソースの作成に使用されます。 `Set`動詞は、既存のリソースを変更するために使用されます。必要に応じて、コマンドレットなどのリソースが存在しない場合は作成し `Set-Variable` ます。

### <a name="find-vs-search"></a>検索と検索

`Find`動詞は、オブジェクトを検索するために使用されます。 `Search`動詞は、コンテナー内のリソースへの参照を作成するために使用されます。
