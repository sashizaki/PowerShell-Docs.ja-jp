...
管理 OData web サービスが公開するリソースを定義するには、パブリックリソーススキーマを使用する MOF ファイルを作成します。 各リソースはファイル内のクラスとして定義され、プロパティはクラスメンバーとして定義されます。 MOF ファイルで使用されるスキーマの詳細については、「[パブリックリソーススキーマ](./public-resource-schema.md)」を参照してください。
...
2つの各リソースには、複合型のプロパティが含まれています。 複合型は、@no__t 0 修飾子を使用して変更されたクラスとして定義されます。
