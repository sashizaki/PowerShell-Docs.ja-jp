---
title: 管理 OData web サービスのカスタム承認を実装する |Microsoft Docs
ms.custom: ''
ms.date: 09/13/2016
ms.reviewer: ''
ms.suite: ''
...
ms.locfileid: "83557296"
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="19d5e-102">Management OData Web サービスのカスタム認可を実装する</span><span class="sxs-lookup"><span data-stu-id="19d5e-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="19d5e-103">Windows PowerShell Web サービスを使用するには、サードパーティが[CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)インターフェイスを実装して windows powershell コマンドレットを公開する必要があります。</span><span class="sxs-lookup"><span data-stu-id="19d5e-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="19d5e-104">このインターフェイスは、web サービスに対するユーザー承認を実行します。</span><span class="sxs-lookup"><span data-stu-id="19d5e-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="19d5e-105">インターフェイスを実装するコードを記述したら、そのコードをコンパイルして、web アプリケーションで使用する DLL を作成する必要があります。</span><span class="sxs-lookup"><span data-stu-id="19d5e-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="19d5e-106">パススルー承認</span><span class="sxs-lookup"><span data-stu-id="19d5e-106">Pass-through authorization</span></span>

<span data-ttu-id="19d5e-107">[CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)インターフェイスを実装する最も簡単な方法は、すべてのユーザーを承認するパススルー実装です。</span><span class="sxs-lookup"><span data-stu-id="19d5e-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="19d5e-108">この例では、セキュリティは提供されず、はインターフェイスを実装する方法の図解としてのみ提供されます。</span><span class="sxs-lookup"><span data-stu-id="19d5e-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="19d5e-109">[CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)インターフェイスを実装するには、 [CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)と CustomAuthorization の2つのメソッドをオーバーライドする必要があります。このような場合には、 [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId)を指定します。</span><span class="sxs-lookup"><span data-stu-id="19d5e-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="19d5e-110">この例では、 [CustomAuthorization ユーザー](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)は常に、現在のユーザーに関連付けられている**WindowsIdentity**オブジェクトを返しています。</span><span class="sxs-lookup"><span data-stu-id="19d5e-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="19d5e-111">ロール ベースの承認</span><span class="sxs-lookup"><span data-stu-id="19d5e-111">Role-based authorization</span></span>

<span data-ttu-id="19d5e-112">次の例では、ロールベースの承認ポリシーを実装します。</span><span class="sxs-lookup"><span data-stu-id="19d5e-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="19d5e-113">ポリシーは、web.config および MOF および XML マッピングスキーマファイルを使用して、メインアプリケーションディレクトリに存在する XML ファイルで定義されます。</span><span class="sxs-lookup"><span data-stu-id="19d5e-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="19d5e-114">承認スキーマファイルを構成する方法の詳細については、「[ロールベースの承認を構成](./configuring-role-based-authorization.md)する」を参照してください。</span><span class="sxs-lookup"><span data-stu-id="19d5e-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="19d5e-115">このサンプルの最初の部分では、 [CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)と[CustomAuthorization の id](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId)メソッドを実装しています。この例を示します。</span><span class="sxs-lookup"><span data-stu-id="19d5e-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="19d5e-116">この場合、インターフェイスメソッドは、 `RbacSystem` ユーザーのアクセス許可を確認する実際の作業を行うクラス (以下で定義) のメソッドを呼び出します。</span><span class="sxs-lookup"><span data-stu-id="19d5e-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="19d5e-117">次の例では、XML ファイル内の承認ポリシーを解析するクラスを作成します。</span><span class="sxs-lookup"><span data-stu-id="19d5e-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="19d5e-118">次のクラスは、それぞれグループとユーザーを表します。</span><span class="sxs-lookup"><span data-stu-id="19d5e-118">The following classes represent groups and users, respectively.</span></span>
