...
## <a name="where-can-i-install-powershell"></a><span data-ttu-id="6a25e-116">PowerShell をインストールできる場所</span><span class="sxs-lookup"><span data-stu-id="6a25e-116">Where can I install PowerShell?</span></span>
...
## <a name="improved-backwards-compatibility-with-windows-powershell"></a><span data-ttu-id="6a25e-146">Windows PowerShell との下位互換性の向上</span><span class="sxs-lookup"><span data-stu-id="6a25e-146">Improved backwards compatibility with Windows PowerShell</span></span>
...
<span data-ttu-id="6a25e-157">**Parallel** パラメーターは、入力ログ名ごとに並列実行されるスクリプト ブロックを指定します。</span><span class="sxs-lookup"><span data-stu-id="6a25e-157">The **Parallel** parameter specifies the script block that is run in parallel for each input log name.</span></span>
...
<span data-ttu-id="6a25e-172">詳細については、「[About If](/powershell/module/microsoft.powershell.core/about/about_if)」を参照してください。</span><span class="sxs-lookup"><span data-stu-id="6a25e-172">For more information [About If](/powershell/module/microsoft.powershell.core/about/about_if).</span></span>
...
<span data-ttu-id="6a25e-191">次の例では、右側のオペランドは評価されません。</span><span class="sxs-lookup"><span data-stu-id="6a25e-191">In the following example, the right-hand operand won't be evaluated:</span></span>
...
<span data-ttu-id="6a25e-221">`Get-Error` コマンドレットは、組み込み変数 `$Error` を使用して、パイプラインからの入力をサポートします。</span><span class="sxs-lookup"><span data-stu-id="6a25e-221">The `Get-Error` cmdlet supports input from the pipeline using the built-in variable `$Error`.</span></span>
...
<span data-ttu-id="6a25e-234">更新通知動作は、`$Env:POWERSHELL_UPDATECHECK` 環境変数を使用して変更できます。</span><span class="sxs-lookup"><span data-stu-id="6a25e-234">The update notification behavior can be changed using the `$Env:POWERSHELL_UPDATECHECK` environment variable.</span></span> <span data-ttu-id="6a25e-235">サポートされている値を次に示します。</span><span class="sxs-lookup"><span data-stu-id="6a25e-235">The following values are supported:</span></span>
...
<span data-ttu-id="6a25e-248">`Invoke-DscResource` コマンドレットは、指定された PowerShell Desired State Configuration (DSC) リソースのメソッドを実行します。</span><span class="sxs-lookup"><span data-stu-id="6a25e-248">The `Invoke-DscResource` cmdlet runs a method of a specified PowerShell Desired State Configuration (DSC) resource.</span></span>
...
### <a name="engine-updates-and-fixes"></a><span data-ttu-id="6a25e-262">エンジンの更新と修正</span><span class="sxs-lookup"><span data-stu-id="6a25e-262">Engine Updates and Fixes</span></span>
...
### <a name="general-cmdlet-updates-and-fixes"></a><span data-ttu-id="6a25e-283">一般的なコマンドレットの更新と修正</span><span class="sxs-lookup"><span data-stu-id="6a25e-283">General Cmdlet Updates and Fixes</span></span>
...
- <span data-ttu-id="6a25e-303">大文字と小文字が正しく区別されるようにパスを再構築するとき、ファイル列挙が失敗した場合に例外を処理 (#11014)</span><span class="sxs-lookup"><span data-stu-id="6a25e-303">Handle exception if enumerating files fails when rebuilding path to have correct casing (#11014)</span></span>
...
- <span data-ttu-id="6a25e-325">アクセント カラーをサポートしていないホストについて、そのアクセント カラーの使用を修正 (#10937)</span><span class="sxs-lookup"><span data-stu-id="6a25e-325">Fix use of accent colors for hosts that don't support them (#10937)</span></span>
...
- <span data-ttu-id="6a25e-346">呼び出しスクリプトの結果として同じ場所へのスクリプトを実行している各 ForEach-Object -Parallel に現在の作業ディレクトリを設定</span><span class="sxs-lookup"><span data-stu-id="6a25e-346">Set current working directory of each ForEach-Object -Parallel running script to the same location as the calling script.</span></span> <span data-ttu-id="6a25e-347">(#10672)</span><span class="sxs-lookup"><span data-stu-id="6a25e-347">(#10672)</span></span>
...
- <span data-ttu-id="6a25e-367">NVDA スクリーン リーダーがアクティブの場合、Windows で PSReadLine の自動読み込みをスキップ (#10385)</span><span class="sxs-lookup"><span data-stu-id="6a25e-367">Skip auto-loading PSReadLine on Windows if the NVDA screen reader is active (#10385)</span></span>
...
- <span data-ttu-id="6a25e-387">デイリー ビルドに対する更新通知チェックを無効化 (#10903) (@bergmeister に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-387">Disable update notification check for daily builds (#10903) (Thanks @bergmeister!)</span></span>
...
- <span data-ttu-id="6a25e-398">ActionPreference.Suspend 列挙値をサポートされていない予約済み状態に変換し、ユーザー設定変数で ActionPreference.Ignore を使用するときの制限を削除 (#10317) (@KirkMunro に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-398">Convert ActionPreference.Suspend enumeration value into a non-supported, reserved state, and remove restriction on using ActionPreference.Ignore in preference variables (#10317) (Thanks @KirkMunro!)</span></span>
- <span data-ttu-id="6a25e-399">機能を変更することなくコードの読みやすさと信頼性を高めるために ArrayList を List\<T> に置換 (#10333) (@iSazonov に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-399">Replace ArrayList with List\<T> to get more readable and reliable code without changing functionality (#10333) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="6a25e-400">TestConnectionCommand へのコード スタイルの修正 (#10439) (@vexx32 に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-400">Make code style fixes to TestConnectionCommand (#10439) (Thanks @vexx32!)</span></span>
- <span data-ttu-id="6a25e-401">AutomationEngine をクリーンアップし、余分な SetSessionStateDrive メソッド呼び出しを削除 (#10416) (@iSazonov に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-401">Cleanup AutomationEngine and remove extra SetSessionStateDrive method call (#10416) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="6a25e-402">既定の ParameterSetName の名前を ConvertTo-Csv および ConvertFrom-Csv の Delimiter に再度変更 (#10425)</span><span class="sxs-lookup"><span data-stu-id="6a25e-402">Rename default ParameterSetName back to Delimiter for ConvertTo-Csv and ConvertFrom-Csv (#10425)</span></span>

### <a name="tools"></a><span data-ttu-id="6a25e-403">ツール</span><span class="sxs-lookup"><span data-stu-id="6a25e-403">Tools</span></span>

- <span data-ttu-id="6a25e-404">VS でビルドが行われるように SDKToUse プロパティの既定の設定を追加 (#11085)</span><span class="sxs-lookup"><span data-stu-id="6a25e-404">Add default setting for the SDKToUse property so that it builds in VS (#11085)</span></span>
- <span data-ttu-id="6a25e-405">Install-Powershell.ps1: MSI インストールが使用されるようにパラメーターを追加 (#10921) (@MJECloud に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-405">Install-Powershell.ps1: Add parameter to use MSI installation (#10921) (Thanks @MJECloud!)</span></span>
- <span data-ttu-id="6a25e-406">install-powershell.ps1 の基本的な例を追加 (#10914) (@kilasuit に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-406">Add basic examples for install-powershell.ps1 (#10914) (Thanks @kilasuit!)</span></span>
- <span data-ttu-id="6a25e-407">Install-PowerShellRemoting.ps1 による PowerShellHome パラメーターの空の文字列の処理を実現 (#10526) (@Orca88 に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-407">Make Install-PowerShellRemoting.ps1 handle empty string in PowerShellHome parameter (#10526) (Thanks @Orca88!)</span></span>
- <span data-ttu-id="6a25e-408">Install-powershell.sh で /etc/lsb-release から /etc/os-release に切り替え (#10773) (@Himura2la に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-408">Switch from /etc/lsb-release to /etc/os-release in install-powershell.sh (#10773) (Thanks @Himura2la!)</span></span>
- <span data-ttu-id="6a25e-409">Windows で pwsh.exe と pwsh のデイリー バージョンを確認 (#10738) (@centreboard に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-409">Check pwsh.exe and pwsh in daily version on Windows (#10738) (Thanks @centreboard!)</span></span>
- <span data-ttu-id="6a25e-410">installpsh-osx.sh で不要なタップを削除 (#10752)</span><span class="sxs-lookup"><span data-stu-id="6a25e-410">Remove unneeded tap in installpsh-osx.sh (#10752)</span></span>
- <span data-ttu-id="6a25e-411">既にインストールされているデイリー ビルド チェックのために install-powershell.ps1 を更新 (#10489)</span><span class="sxs-lookup"><span data-stu-id="6a25e-411">Update install-powershell.ps1 to check for already installed daily build (#10489)</span></span>

### <a name="tests"></a><span data-ttu-id="6a25e-412">テスト</span><span class="sxs-lookup"><span data-stu-id="6a25e-412">Tests</span></span>

- <span data-ttu-id="6a25e-413">信頼性の低い DSC テストを保留 (#11131)</span><span class="sxs-lookup"><span data-stu-id="6a25e-413">Make unreliable DSC test pending (#11131)</span></span>
- <span data-ttu-id="6a25e-414">ハッシュテーブルのキーが正しく検証されるように stringdata テストを修正 (#10810)</span><span class="sxs-lookup"><span data-stu-id="6a25e-414">Fix stringdata test to correctly validate keys of hashtables (#10810)</span></span>
- <span data-ttu-id="6a25e-415">テスト モジュールをアンロード (#11061) (@iSazonov に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-415">Unload test modules (#11061) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="6a25e-416">URL テストの再試行間隔を長くする (#11015)</span><span class="sxs-lookup"><span data-stu-id="6a25e-416">Increase time between retries of testing URL (#11015)</span></span>
- <span data-ttu-id="6a25e-417">テスト アクションが正確に記述されるようにテストを更新</span><span class="sxs-lookup"><span data-stu-id="6a25e-417">Update tests to accurately describe test actions.</span></span> <span data-ttu-id="6a25e-418">(#10928) (@romero126 に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-418">(#10928) (Thanks @romero126!)</span></span>
- <span data-ttu-id="6a25e-419">不安定なテスト TestAppDomainProcessExitEvenHandlerNotLeaking を一時的にスキップ (#10827)</span><span class="sxs-lookup"><span data-stu-id="6a25e-419">Temporary skip the flaky test TestAppDomainProcessExitEvenHandlerNotLeaking (#10827)</span></span>
- <span data-ttu-id="6a25e-420">イベント ハンドラー リーク テストの安定化 (#10790)</span><span class="sxs-lookup"><span data-stu-id="6a25e-420">Make the event handler leaking test stable (#10790)</span></span>
- <span data-ttu-id="6a25e-421">CI YAML の大文字と小文字の区別を同期 (#10767) (@RDIL に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-421">Sync capitalization in CI YAML (#10767) (Thanks @RDIL!)</span></span>
- <span data-ttu-id="6a25e-422">イベント ハンドラー リーク修正のテストを追加 (#10768)</span><span class="sxs-lookup"><span data-stu-id="6a25e-422">Add test for the event handler leaking fix (#10768)</span></span>
- <span data-ttu-id="6a25e-423">Get-ChildItem テストを追加 (#10507) (@iSazonov に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-423">Add Get-ChildItem test (#10507) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="6a25e-424">正確性を高めるためにテストのあいまいな言語をスイッチからパラメーターに置換 (#10666) (@romero126 に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-424">Replace ambiguous language for tests from switch to parameter for accuracy (#10666) (Thanks @romero126!)</span></span>
- <span data-ttu-id="6a25e-425">試験的なチェックを ForEach-Object -Parallel テストに追加 (#10354) (@KirkMunro に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-425">Add experimental check to ForEach-Object -Parallel tests (#10354) (Thanks @KirkMunro!)</span></span>
- <span data-ttu-id="6a25e-426">Alpine 検証のテストを更新 (#10428)</span><span class="sxs-lookup"><span data-stu-id="6a25e-426">Update tests for Alpine validation (#10428)</span></span>

### <a name="build-and-package-improvements"></a><span data-ttu-id="6a25e-427">ビルドとパッケージの機能強化</span><span class="sxs-lookup"><span data-stu-id="6a25e-427">Build and Package Improvements</span></span>

- <span data-ttu-id="6a25e-428">調整済みパッケージ ビルドの Nuget パッケージ署名を修正 (#11316)</span><span class="sxs-lookup"><span data-stu-id="6a25e-428">Fix Nuget package signing for Coordinated Package build (#11316)</span></span>
- <span data-ttu-id="6a25e-429">PowerShell ギャラリーと NuGet からの依存関係を更新 (#11323)</span><span class="sxs-lookup"><span data-stu-id="6a25e-429">Update dependencies from PowerShell Gallery and NuGet (#11323)</span></span>
- <span data-ttu-id="6a25e-430">Microsoft.ApplicationInsights を 2.11.0 から 2.12.0 にバージョンアップ (#11305)</span><span class="sxs-lookup"><span data-stu-id="6a25e-430">Bump Microsoft.ApplicationInsights from 2.11.0 to 2.12.0 (#11305)</span></span>
- <span data-ttu-id="6a25e-431">Microsoft.CodeAnalysis.CSharp を 3.3.1 から 3.4.0 にバージョンアップ (#11265)</span><span class="sxs-lookup"><span data-stu-id="6a25e-431">Bump Microsoft.CodeAnalysis.CSharp from 3.3.1 to 3.4.0 (#11265)</span></span>
- <span data-ttu-id="6a25e-432">Debian 10 および 11 のパッケージを更新 (#11236)</span><span class="sxs-lookup"><span data-stu-id="6a25e-432">Updates packages for Debian 10 and 11 (#11236)</span></span>
- <span data-ttu-id="6a25e-433">RC より前の試験的な機能のみを有効化 (#11162)</span><span class="sxs-lookup"><span data-stu-id="6a25e-433">Only enable experimental features prior to RC (#11162)</span></span>
- <span data-ttu-id="6a25e-434">macOS の最小バージョンを更新 (#11163)</span><span class="sxs-lookup"><span data-stu-id="6a25e-434">Update macOS minimum version (#11163)</span></span>
- <span data-ttu-id="6a25e-435">NJsonSchema を 10.0.27 から 10.0.28 にバージョンアップ (#11170)</span><span class="sxs-lookup"><span data-stu-id="6a25e-435">Bump NJsonSchema from 10.0.27 to 10.0.28 (#11170)</span></span>
- <span data-ttu-id="6a25e-436">Preview.5 の README.md および metadata.json のリンクを更新 (#10854)</span><span class="sxs-lookup"><span data-stu-id="6a25e-436">Updating links in README.md and metadata.json for Preview.5 (#10854)</span></span>
- <span data-ttu-id="6a25e-437">PowerShell 所有のコンプライアンス テストのファイルを選択 (#10837)</span><span class="sxs-lookup"><span data-stu-id="6a25e-437">Select the files for compliance tests which are owned by PowerShell (#10837)</span></span>
- <span data-ttu-id="6a25e-438">win7x86 msix パッケージによるビルドを許可</span><span class="sxs-lookup"><span data-stu-id="6a25e-438">Allow win7x86 msix package to build.</span></span> <span data-ttu-id="6a25e-439">(内部 10515)</span><span class="sxs-lookup"><span data-stu-id="6a25e-439">(Internal 10515)</span></span>
- <span data-ttu-id="6a25e-440">セマンティク バージョンを NormalizeVersion 関数に渡すことを許可 (#11087)</span><span class="sxs-lookup"><span data-stu-id="6a25e-440">Allow semantic versions to be passed to NormalizeVersion function (#11087)</span></span>
- <span data-ttu-id="6a25e-441">.NET Core Framework を 3.1-preview.3 にバージョンアップ (#11079)</span><span class="sxs-lookup"><span data-stu-id="6a25e-441">Bump .NET core framework to 3.1-preview.3 (#11079)</span></span>
- <span data-ttu-id="6a25e-442">/src/Modules で PSReadLine を 2.0.0-beta5 から 2.0.0-beta6 にバージョンアップ (#11078)</span><span class="sxs-lookup"><span data-stu-id="6a25e-442">Bump PSReadLine from 2.0.0-beta5 to 2.0.0-beta6 in /src/Modules (#11078)</span></span>
- <span data-ttu-id="6a25e-443">Newtonsoft.Json を 12.0.2 から 12.0.3 にバージョンアップ (#11037) (#11038)</span><span class="sxs-lookup"><span data-stu-id="6a25e-443">Bump Newtonsoft.Json from 12.0.2 to 12.0.3 (#11037) (#11038)</span></span>
- <span data-ttu-id="6a25e-444">Debian 10、11、および CentOS 8 パッケージを追加 (#11028)</span><span class="sxs-lookup"><span data-stu-id="6a25e-444">Add Debian 10, 11 and CentOS 8 packages (#11028)</span></span>
- <span data-ttu-id="6a25e-445">ReleaseDate フィールドを含む Build-Info Json ファイルをアップロード (#10986)</span><span class="sxs-lookup"><span data-stu-id="6a25e-445">Upload Build-Info Json file with the ReleaseDate field (#10986)</span></span>
- <span data-ttu-id="6a25e-446">.NET Core Framework を 3.1-preview.2 にバージョンアップ (#10993)</span><span class="sxs-lookup"><span data-stu-id="6a25e-446">Bump .NET core framework to 3.1-preview.2 (#10993)</span></span>
- <span data-ttu-id="6a25e-447">x86 MSIX パッケージのビルドを有効化 (#10934)</span><span class="sxs-lookup"><span data-stu-id="6a25e-447">Enable build of x86 MSIX package (#10934)</span></span>
- <span data-ttu-id="6a25e-448">build.psm1 の dotnet SDK インストール スクリプト URL を更新 (#10927)</span><span class="sxs-lookup"><span data-stu-id="6a25e-448">Update the dotnet SDK install script URL in build.psm1 (#10927)</span></span>
- <span data-ttu-id="6a25e-449">Markdig.Signed を 0.17.1 から 0.18.0 にバージョンアップ (#10887)</span><span class="sxs-lookup"><span data-stu-id="6a25e-449">Bump Markdig.Signed from 0.17.1 to 0.18.0 (#10887)</span></span>
- <span data-ttu-id="6a25e-450">ThreadJob を 2.0.1 から 2.0.2 にバージョンアップ (#10886)</span><span class="sxs-lookup"><span data-stu-id="6a25e-450">Bump ThreadJob from 2.0.1 to 2.0.2 (#10886)</span></span>
- <span data-ttu-id="6a25e-451">MS Store の要件を満たすように AppX マニフェストおよびパッケージ モジュールを更新 (#10878)</span><span class="sxs-lookup"><span data-stu-id="6a25e-451">Update AppX Manifest and Packaging module to conform to MS Store requirements (#10878)</span></span>
- <span data-ttu-id="6a25e-452">PowerShell SDK のパッケージ参照を preview.5 に更新 (内部 10295)</span><span class="sxs-lookup"><span data-stu-id="6a25e-452">Update package reference for PowerShell SDK to preview.5 (Internal 10295)</span></span>
- <span data-ttu-id="6a25e-453">ThirdPartyNotices.txt を更新 (#10834)</span><span class="sxs-lookup"><span data-stu-id="6a25e-453">Update ThirdPartyNotices.txt (#10834)</span></span>
- <span data-ttu-id="6a25e-454">Microsoft.PowerShell.Native を 7.0.0-preview.3 にバージョンアップ (#10826)</span><span class="sxs-lookup"><span data-stu-id="6a25e-454">Bump Microsoft.PowerShell.Native to 7.0.0-preview.3 (#10826)</span></span>
- <span data-ttu-id="6a25e-455">Microsoft.ApplicationInsights を 2.10.0 から 2.11.0 にバージョンアップ (#10608)</span><span class="sxs-lookup"><span data-stu-id="6a25e-455">Bump Microsoft.ApplicationInsights from 2.10.0 to 2.11.0 (#10608)</span></span>
- <span data-ttu-id="6a25e-456">NJsonSchema を 10.0.24 から 10.0.27 にバージョンアップ (#10756)</span><span class="sxs-lookup"><span data-stu-id="6a25e-456">Bump NJsonSchema from 10.0.24 to 10.0.27 (#10756)</span></span>
- <span data-ttu-id="6a25e-457">MacPorts サポートをビルド システムに追加 (#10736) (@Lucius-Q-User に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-457">Add MacPorts support to the build system (#10736) (Thanks @Lucius-Q-User!)</span></span>
- <span data-ttu-id="6a25e-458">PackageManagement を 1.4.4 から 1.4.5 にバージョンアップ (#10728)</span><span class="sxs-lookup"><span data-stu-id="6a25e-458">Bump PackageManagement from 1.4.4 to 1.4.5 (#10728)</span></span>
- <span data-ttu-id="6a25e-459">NJsonSchema を 10.0.23 から 10.0.24 にバージョンアップ (#10635)</span><span class="sxs-lookup"><span data-stu-id="6a25e-459">Bump NJsonSchema from 10.0.23 to 10.0.24 (#10635)</span></span>
- <span data-ttu-id="6a25e-460">MSI でクライアント/サーバーのテレメトリを区別できるように環境変数を追加 (#10612)</span><span class="sxs-lookup"><span data-stu-id="6a25e-460">Add environment variable to differentiate client/server telemetry in MSI (#10612)</span></span>
- <span data-ttu-id="6a25e-461">PSDesiredStateConfiguration を 2.0.3 から 2.0.4 にバージョンアップ (#10603)</span><span class="sxs-lookup"><span data-stu-id="6a25e-461">Bump PSDesiredStateConfiguration from 2.0.3 to 2.0.4 (#10603)</span></span>
- <span data-ttu-id="6a25e-462">Microsoft.CodeAnalysis.CSharp を 3.2.1 から 3.3.1 にバージョンアップ (#10607)</span><span class="sxs-lookup"><span data-stu-id="6a25e-462">Bump Microsoft.CodeAnalysis.CSharp from 3.2.1 to 3.3.1 (#10607)</span></span>
- <span data-ttu-id="6a25e-463">.Net Core 3.0 RTM に更新 (#10604) (@bergmeister に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-463">Update to .Net Core 3.0 RTM (#10604) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="6a25e-464">Windows ストアの要件に対応するように MSIX パッケージを更新 (#10588)</span><span class="sxs-lookup"><span data-stu-id="6a25e-464">Update MSIX packaging so the version to Windows Store requirements (#10588)</span></span>
- <span data-ttu-id="6a25e-465">PowerShellGet を 2.2 から 2.2.1 にバージョンアップ (#10382)</span><span class="sxs-lookup"><span data-stu-id="6a25e-465">Bump PowerShellGet version from 2.2 to 2.2.1 (#10382)</span></span>
- <span data-ttu-id="6a25e-466">PackageManagement を 1.4.3 から 1.4.4 にバージョンアップ (#10383)</span><span class="sxs-lookup"><span data-stu-id="6a25e-466">Bump PackageManagement version from 1.4.3 to 1.4.4 (#10383)</span></span>
- <span data-ttu-id="6a25e-467">7\.0.0-preview.4 の README.md および metadata.json を更新 (内部 10011)</span><span class="sxs-lookup"><span data-stu-id="6a25e-467">Update README.md and metadata.json for 7.0.0-preview.4 (Internal 10011)</span></span>
- <span data-ttu-id="6a25e-468">.Net Core 3.0 バージョンを Preview 9 から RC1 にアップグレード (#10552) (@bergmeister に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-468">Upgrade .Net Core 3.0 version from Preview 9 to RC1 (#10552) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="6a25e-469">ExperimentalFeature リスト生成を修正 (内部 9996)</span><span class="sxs-lookup"><span data-stu-id="6a25e-469">Fix ExperimentalFeature list generation (Internal 9996)</span></span>
- <span data-ttu-id="6a25e-470">PSReadLine を 2.0.0-beta4 から 2.0.0-beta5 にバージョンアップ (#10536)</span><span class="sxs-lookup"><span data-stu-id="6a25e-470">Bump PSReadLine version from 2.0.0-beta4 to 2.0.0-beta5 (#10536)</span></span>
- <span data-ttu-id="6a25e-471">リリース タグが設定されるようにリリース ビルド スクリプトを修正</span><span class="sxs-lookup"><span data-stu-id="6a25e-471">Fix release build script to set release tag</span></span>
- <span data-ttu-id="6a25e-472">Microsoft.PowerShell.Native のバージョンを 7.0.0-preview.2 に更新 (#10519)</span><span class="sxs-lookup"><span data-stu-id="6a25e-472">Update version of Microsoft.PowerShell.Native to 7.0.0-preview.2 (#10519)</span></span>
- <span data-ttu-id="6a25e-473">Netcoreapp3.0 preview9 をアップグレード (#10484) (@bergmeister に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-473">Upgrade to Netcoreapp3.0 preview9 (#10484) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="6a25e-474">調整済みデイリー ビルドがデイリー ビルドであることを確実に認識 (#10464)</span><span class="sxs-lookup"><span data-stu-id="6a25e-474">Make sure the daily coordinated build, knows it is a daily build (#10464)</span></span>
- <span data-ttu-id="6a25e-475">デイリー ビルドをリリースするように結合パッケージ ビルドを更新 (#10449)</span><span class="sxs-lookup"><span data-stu-id="6a25e-475">Update the combined package build to release the daily builds (#10449)</span></span>
- <span data-ttu-id="6a25e-476">appveyor 参照を削除 (#10445) (@RDIL に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-476">Remove appveyor reference (#10445) (Thanks @RDIL!)</span></span>
- <span data-ttu-id="6a25e-477">NJsonSchema を 10.0.22 から 10.0.23 にバージョンアップ (#10421)</span><span class="sxs-lookup"><span data-stu-id="6a25e-477">Bump NJsonSchema version from 10.0.22 to 10.0.23 (#10421)</span></span>
- <span data-ttu-id="6a25e-478">Alpine の依存関係の一部で必要とされるため linux-x64 ビルド フォルダーの削除を削除 (#10407)</span><span class="sxs-lookup"><span data-stu-id="6a25e-478">Remove the deletion of linux-x64 build folder because some dependencies for Alpine need it (#10407)</span></span>

### <a name="documentation-and-help-content"></a><span data-ttu-id="6a25e-479">ドキュメントとヘルプ コンテンツ</span><span class="sxs-lookup"><span data-stu-id="6a25e-479">Documentation and Help Content</span></span>

- <span data-ttu-id="6a25e-480">変更ログをリリースごとに 1 つのログにリファクター (#11165)</span><span class="sxs-lookup"><span data-stu-id="6a25e-480">Refactor change logs into one log per release (#11165)</span></span>
- <span data-ttu-id="6a25e-481">PowerShell 7 オンライン ヘルプ ドキュメントの FWLink を修正 (#11071)</span><span class="sxs-lookup"><span data-stu-id="6a25e-481">Fix FWLinks for PowerShell 7 online help documents (#11071)</span></span>
- <span data-ttu-id="6a25e-482">CONTRIBUTING.md を更新 (#11096) (@mklement0 に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-482">Update CONTRIBUTING.md (#11096) (Thanks @mklement0!)</span></span>
- <span data-ttu-id="6a25e-483">README.md のインストール ドキュメントのリンクを修正 (#11083)</span><span class="sxs-lookup"><span data-stu-id="6a25e-483">Fix installation doc links in README.md (#11083)</span></span>
- <span data-ttu-id="6a25e-484">install-powershell.ps1 スクリプトに例を追加 (#11024) (@kilasuit に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-484">Adds examples to install-powershell.ps1 script (#11024) (Thanks @kilasuit!)</span></span>
- <span data-ttu-id="6a25e-485">CHANGELOG.md の Select-String 強調および Import-DscResource への修正 (#10890)</span><span class="sxs-lookup"><span data-stu-id="6a25e-485">Fix to Select-String emphasis and Import-DscResource in CHANGELOG.md (#10890)</span></span>
- <span data-ttu-id="6a25e-486">powershell-beginners-guide.md から古いリンクを削除 (#10926)</span><span class="sxs-lookup"><span data-stu-id="6a25e-486">Remove the stale link from powershell-beginners-guide.md (#10926)</span></span>
- <span data-ttu-id="6a25e-487">安定したサービス変更ログの統合 (#10527)</span><span class="sxs-lookup"><span data-stu-id="6a25e-487">Merge stable and servicing change logs (#10527)</span></span>
- <span data-ttu-id="6a25e-488">ビルド ドキュメントの使用されている .NET バージョンを更新 (#10775) (@Greg-Smulko に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-488">Update used .NET version in build docs (#10775) (Thanks @Greg-Smulko!)</span></span>
- <span data-ttu-id="6a25e-489">powershell-beginners-guide.md で MSDN のリンクを docs.microsoft.com に置換 (#10778) (@iSazonov に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-489">Replace links from MSDN to docs.microsoft.com in powershell-beginners-guide.md (#10778) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="6a25e-490">破損した DSC 概要リンクを修正 (#10702)</span><span class="sxs-lookup"><span data-stu-id="6a25e-490">Fix broken DSC overview link (#10702)</span></span>
- <span data-ttu-id="6a25e-491">Support_Question.md を更新して、Stack Overflow に別のコミュニティ リソースとしてリンク (#10638) (@mklement0 に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-491">Update Support_Question.md to link to Stack Overflow as another community resource (#10638) (Thanks @mklement0!)</span></span>
- <span data-ttu-id="6a25e-492">プロセッサ アーキテクチャをディストリビューション要求テンプレートに追加 (#10661)</span><span class="sxs-lookup"><span data-stu-id="6a25e-492">Add processor architecture to distribution request template (#10661)</span></span>
- <span data-ttu-id="6a25e-493">新しい PowerShell MoL ブックを学習用の PowerShell ドキュメントに追加 (#10602)</span><span class="sxs-lookup"><span data-stu-id="6a25e-493">Add new PowerShell MoL book to learning PowerShell docs (#10602)</span></span>
- <span data-ttu-id="6a25e-494">v6.1.6 および v6.2.3 リリースの README.md およびメタデータを更新 (#10523)</span><span class="sxs-lookup"><span data-stu-id="6a25e-494">Update README.md and metadata for v6.1.6 and v6.2.3 releases (#10523)</span></span>
- <span data-ttu-id="6a25e-495">README.md の入力ミスを修正 (#10465) (@vedhasp に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-495">Fix a typo in README.md (#10465) (Thanks @vedhasp!)</span></span>
- <span data-ttu-id="6a25e-496">PSKoans モジュールへの参照を学習用のリソース ドキュメントに追加 (#10369) (@vexx32 に感謝)</span><span class="sxs-lookup"><span data-stu-id="6a25e-496">Add a reference to PSKoans module to Learning Resources documentation (#10369) (Thanks @vexx32!)</span></span>
- <span data-ttu-id="6a25e-497">7\.0.0-preview.3 の README.md および metadata.json を更新 (#10393)</span><span class="sxs-lookup"><span data-stu-id="6a25e-497">Update README.md and metadata.json for 7.0.0-preview.3 (#10393)</span></span>
