...
---
# <a name="powershell-script-discovery-install-and-management-with-powershellget"></a><span data-ttu-id="db7c5-102">PowerShellGet による PowerShell スクリプトの検出、インストール、および管理</span><span class="sxs-lookup"><span data-stu-id="db7c5-102">PowerShell Script Discovery, Install and Management with PowerShellGet</span></span>

<span data-ttu-id="db7c5-103">WMF 5.0 RTM リリースでは、PowerShell スクリプト共有機能が PowerShellGet に追加されました。</span><span class="sxs-lookup"><span data-stu-id="db7c5-103">PowerShell Script sharing feature got added to the PowerShellGet in the WMF 5.0 RTM release.</span></span>
<span data-ttu-id="db7c5-104">次の新しいコマンドレットが PowerShell スクリプトをサポートするために PowerShellGet モジュールに追加されます。</span><span class="sxs-lookup"><span data-stu-id="db7c5-104">The following new cmdlets are added to the PowerShellGet module for supporting the PowerShell Scripts.</span></span>
```powershell
...
<span data-ttu-id="db7c5-105">スクリプト共有コマンドレットの構文は次のとおりです。</span><span class="sxs-lookup"><span data-stu-id="db7c5-105">Syntax of script sharing Cmdlets:</span></span>
```powershell
